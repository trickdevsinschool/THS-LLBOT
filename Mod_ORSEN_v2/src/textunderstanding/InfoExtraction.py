...
def event_extraction(sentence, world, current_node):
    ...
    for i in range(0, len(sentence.dep)):
        ...
        if sentence.dep[i] == 'aux' or sentence.dep[i] == 'auxpass' and (i + 1) < len(sentence.dep):
            ...
            i] != 'advcl' and sentence.dep[i] != 'pcomp' and sentence.dep[i] != 'acomp' and sentence.dep[
            i] != 'xcomp' and i > 1:  # (jilyan) added i>1 bec error
                                ...
                                if sentence.dep[i + k] == 'conj':
                                    ...
                                    if sentence.head_text[i + k] == test_obj or sentence.head_text[i + k] == ',':
                                        ...
                                        if sentence.dep[i + k - 1] == 'neg' or sentence.dep[i + k - 1] == 'aux':
                                            event_attr[len(event_attr) - 1] += "," + sentence.text_token[
                                                i + k - 1] + " " + test_obj  # this was originally event_attr[x] but i changed it because there is no x and it keeps getting errors - jilyan
                                        elif len(event_attr) > 0:
                                            event_attr[len(
                                                event_attr) - 1] += "," + test_obj  # this was originally event_attr[x] but i changed it because there is no x and it keeps getting errors - jilyan
                            ...
                            if (i + k) < len(sentence.dep):
                                if sentence.dep[i + k] == 'conj':
                                    if isAdded is False:
                                        ...
                                        if isConnected is False and len(event_dobj) >= len(event_subj_act) and len(
                                                event_subj_act) != 0:
                        ...
                        for k in range(0, len(sentence.dep)):
                            if (i + k) < len(sentence.dep):
                                if sentence.dep[i + k] == 'conj':
                                    ...
                                    if (sentence.head_text[i + k] == test_obj or sentence.head_text[
                                        i + k] == ',') and len(event_attr) > 0:
...
def find_unkown_word(sent):
    ignore = ["kill", "murder", "suicide", "sex", "rape", "war", "torture", "die", "stab", "penis", "dick", "vagina",
              "boobs", "breast",
              "nobody", "none", "every", "any", "some", "other", "something", "anything", "nothing", "everything",
              "all", "this", "that",
...
def extract_relation(sent, world, ie_fileWriter):
    ...
    while first_index < len(sent.dep):
        ...
        if sent.dep[first_index] in templates_first and sent.lemma[first_index] != "-PRON-" and sent.lemma[
            first_index] not in ignore and sent.lemma[first_index] not in be_verb:
            ...
            for c_index in character_index_list:
                ...
                if (temp is None and sent.lemma[c_index] == "-PRON-") or (
                        temp is not None and temp.second == "character"):
            ...
            for second_index in range(first_index + 1, len(sent.dep)):
                ...
                if sent.dep[second_index] in templates_second and sent.lemma[second_index] != "-PRON-" and \
                    ...
                    templates_keyword_type = DBO_Relation.get_keyword_type(sent.dep[first_index],
                                                                           sent.dep[second_index])
                    ...
                    if len(character_index_list) > 1:
                        actual_keywords_dep = sent.dep[
                                              character_index_list[len(character_index_list) - 1] + 1: second_index]
                        actual_keywords_lem = sent.lemma[
                                              character_index_list[len(character_index_list) - 1] + 1: second_index]

                    if "neg" not in actual_keywords_dep and (
                            "no" not in actual_keywords_lem or "not" not in actual_keywords_lem):
                        ...
                        for templates_k_index in range(0, len(templates_keywords)):
                            ...
                            if templates_keywords[templates_k_index] in actual_keywords or (
                                    templates_keywords[templates_k_index] == "" and actual_keywords == []):
                                ...
                                if (temp is None and sent.lemma[second_index] == "-PRON-") or (
                                        temp is not None and temp.second == "character"):
                                    second_word = "character"

                                # save new extracted relation
                                relation = DBO_Relation.get_matching_relation_pattern(sent.dep[first_index],
                                                                                      templates_keywords[
                                                                                          templates_k_index],
                                                                                      sent.dep[second_index])
                                if relation is not None:
                                    ...
                                    for first in character_list:
                                        ...
                                        if relation.is_flipped == 'y':
                                            extracted.append(
                                                Relation.Relation(relation.id, relation.relation, second_word,
                                                                  relation.keywords, first))
                                            print("appended 3: " + extracted[len(extracted) - 1].__str__())
                                            ie_fileWriter.write(
                                                "appended 3: " + extracted[len(extracted) - 1].__str__() + "\n")
                                        else:
                                            extracted.append(Relation.Relation(relation.id, relation.relation, first,
                                                                               relation.keywords, second_word))
                                            print("appended 4: " + extracted[len(extracted) - 1].__str__())
                                            ie_fileWriter.write(
                                                "appended 4: " + extracted[len(extracted) - 1].__str__() + "\n")

                                    if not (relation.relation == "HasProperty" and relation.first == "amod"):
                                        while (second_index + 2 < len(sent.dep) and sent.dep[
                                            second_index + 2] == "conj" and sent.pos[second_index + 2] == sent.pos[
                                                   second_index]) or \
                                                (second_index + 3 < len(sent.dep) and sent.dep[
                                                    second_index + 3] == "conj" and sent.pos[second_index + 3] ==
                                                 sent.pos[second_index]):
                                            second_index += 1
                                            ...
                                            if temp is not None:
                                                ...
                                                ie_fileWriter.write(
                                                    "appended 5: " + extracted[len(extracted) - 1].__str__() + "\n")
                                                if temp.second == "location":
                                                    second_word = temp.first
                                            if (temp is None and sent.lemma[second_index] == "-PRON-") or (
                                                    temp is not None and temp.second == "character"):
                                            ...
                                            for first in character_list:
                                                if relation.is_flipped == 'y':
                                                    ...
                                                    ie_fileWriter.write(
                                                        "appended 6: " + extracted[len(extracted) - 1].__str__() + "\n")
                                                else:
                                                    extracted.append(
                                                        Relation.Relation(relation.id, relation.relation, first,
                                                                          relation.keywords, second_word))
                                                    print("appended 7: " + extracted[len(extracted) - 1].__str__())
                                                    ie_fileWriter.write(
                                                        "appended 7: " + extracted[len(extracted) - 1].__str__() + "\n")
...
def add_relations_to_local(userid, extracted):
    ...
    temp = None
    for i in range(len(extracted)):
        if DBO_Local_Concept.get_concept_specified(extracted[i].first, extracted[i].relation,
                                                   extracted[i].second) == None:
